...
export default function createRenderFunction(_options, helpers, partials, tree) {
    ...
    function compileNodes(nodes) {
        ...
        for (const node of nodes) {
            ...
            if (node.type === 'COMMENT' || node.type === 'BLOCK_CLOSE' || node.type === 'ELSE') {
                continue;
            }
    ...
    function compileMustache(node) {
        ...
        return compileInterpolation(head, true);
    }
    ...
    function compileInterpolation(sub, escape) {
        ...
        // its value directly.
        if (!path) {
            const value = escape ? escapeHTMLChars(sub.value) : coerce(sub.value);
        ...
        if (escape) {
            return function renderEscaped(frame) {
                return escapeHTMLChars(lookup(frame, path));
            };
        }
