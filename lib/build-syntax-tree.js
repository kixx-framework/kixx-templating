...
/**
 * Parses template tokens into an abstract syntax tree used by the renderer.
 *
 * @param {Object|null} _options - Reserved for future use; pass null.
 * @param {Array<Object>} tokens - Tokens returned by tokenize().
 * @returns {Array<Object>} Syntax tree nodes in source order.
 * @throws {LineSyntaxError} If an expression or block is malformed.
 */
export default function buildSyntaxTree(_options, tokens) {
    const tree = [];
    const blocks = [];

    let mustacheOpenToken = null;
    let commentOpenToken = null;
    let rawMustache = false;
    let expressionParts = [];
    ...
    for (const token of tokens) {
        if (token.tokenString === '{{' || token.tokenString === '{{{') {
            ...
            // Triple mustache ({{{ ... }}}) produces unescaped output.
            rawMustache = token.tokenString === '{{{';
        ...
        } else if (mustacheOpenToken && (token.tokenString === '}}' || token.tokenString === '}}}')) {
            addExpression(mustacheOpenToken, token, expressionParts, rawMustache);
            expressionParts = [];
    ...
    function addExpression(openToken, closeToken, parts, raw) {
        ...
        // Triple mustache ({{{ ... }}}) is always an unescaped expression.
        if (raw) {
            pushNode(createExpressionNode(openToken, closeToken, parts, tokenString, false));
            return;
        }
        ...
        if (tokenString.startsWith('#')) {
            pushBlockOpen({
                ...
                children: [],
                tokens: parts,
                standalone: parts[0] ? parts[0].standalone === true : false,
                openToken,
                closeToken,
            });
            ...
            pushNode({
                type: 'PARTIAL',
                exp: tokenString.slice(1).trim(),
                indentation: parts[0] ? parts[0].standaloneIndent : undefined,
                tokens: parts,
                standalone: parts[0] ? parts[0].standalone === true : false,
                openToken,
                closeToken,
            });
        } else if (isSetDelimiterExpression(tokenString)) {
            pushNode({
                type: 'SET_DELIMITERS',
                tokens: parts,
                standalone: parts[0] ? parts[0].standalone === true : false,
            ...
            // Ampersand prefix produces unescaped output.
            pushNode(createExpressionNode(openToken, closeToken, parts, tokenString.slice(1).trim(), false));
        } else if (tokenString.startsWith('!')) {
            // Single-bang comment: {{! ... }} produces no output.
            pushNode({
                type: 'COMMENT',
                tokens: parts,
                standalone: parts[0] ? parts[0].standalone === true : false,
            ...
            pushNode(createExpressionNode(openToken, closeToken, parts, tokenString, true));
        }
    }

    function isSetDelimiterExpression(tokenString) {
        const trimmed = tokenString.trim();
        return trimmed.startsWith('=') && trimmed.endsWith('=');
    }

    function createExpressionNode(openToken, closeToken, parts, tokenString, escape) {
    ...
    function parseSymbols(openToken, closeToken, expressionString) {
        ...
        // Keyword literals carry their original spelling so that, in a name/head
        // position (a section or interpolation target), they can be treated as a
        // property path instead — e.g. Mustache `{{#null}}` looks up the key "null".
        // In an argument position the literal value is used.
    ...
    function parseReferencePath(openToken, _closeToken, refpath) {
        // A lone "." is the Mustache implicit iterator: the current context value.
        // It is represented as an empty path (no segments).
        if (refpath === '.') {
            return [];
        }
        if (refpath[0] === '.') {
    ...
    function validateKeyValueKey(openToken, _closeToken, key) {
        if (/^[\W]+$/.test(key)) {
            throw new LineSyntaxError(
    ...
    function pushBlockClose(node) {
        ...
        if (!openBlock) {
            throw new LineSyntaxError(
                `Unexpected block close "{{/${ node.exp }}}" with no matching open block in ${ node.openToken.filename } on line ${ node.openToken.lineNumber }`,
                node.openToken,
            );
        }

        const openName = blockOpenName(openBlock);

        // A named close tag must match the name of the block it closes. An empty
        // close tag ("{{/}}") is allowed to close any block.
        if (node.exp && openName && node.exp !== openName) {
            throw new LineSyntaxError(
                `Mismatched block close "{{/${ node.exp }}}" does not match block "{{#${ openName }}}" opened in ${ openBlock.openToken.filename } on line ${ openBlock.openToken.lineNumber }`,
                node.openToken,
            );
        }

        openBlock.children.push(node);
        blocks.pop();
    }

    // The comparable name of an open block: the path string of a section/helper
    // head ("each", "user.profile") or a keyword literal in name position ("null").
    function blockOpenName(blockNode) {
        const head = blockNode.exp && blockNode.exp[0];
        if (!head) {
            return null;
        }
        if (head.type === 'PATH') {
            return head.pathString;
        }
        if (head.type === 'LITERAL' && typeof head.keyword === 'string') {
            return head.keyword;
        }
        return null;
    }
